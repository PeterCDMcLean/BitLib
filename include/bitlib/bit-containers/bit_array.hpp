...
constexpr bit_array(std::initializer_list<bit_value> init);
constexpr bit_array(std::initializer_list<bool> init);
constexpr bit_array(std::initializer_list<WordType> init);
constexpr bit_array(std::string_view s);
...
template<std::size_t N>
constexpr bit_array<N>::bit_array(bit_array<N>::value_type bit_val) {
  if (bit_val) {
...
template<std::size_t N>
constexpr bit_array<N>::bit_array(std::initializer_list<bit_value> init) {
  static_assert(init.size() == bitsof(*this));
  std::copy(init.begin(), init.end(), this->begin());
}

template<std::size_t N>
constexpr bit_array<N>::bit_array(std::initializer_list<bool> init) {
  static_assert(init.size() == bitsof(*this));
  std::copy(init.begin(), init.end(), this->begin());
}

template<std::size_t N>
constexpr bit_array<N>::bit_array(std::initializer_list<WordType> init) : storage(init) {
  static_assert(init.size() == storage.size());
}

template<std::size_t N>
constexpr bit_array<N>::bit_array(std::string_view s) {
