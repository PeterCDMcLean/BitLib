...
TYPED_TEST(VectorTest, CopyAssignment) {
    for (auto _ = 0; _ < 128; ++_) {
        ...
        EXPECT_NE(std::to_address(this->empty_vec.begin().base()), std::to_address(this->v2_.begin().base()));
    ...
    for (unsigned int vec_idx = 0; vec_idx < this->random_bitvecs.size(); ++vec_idx) {
      ...
      EXPECT_NE(std::to_address(this->empty_vec.begin().base()), std::to_address(bitvec.begin().base()));
      EXPECT_TRUE(std::equal(
          this->empty_vec.begin(),
          this->empty_vec.end(),
          boolvec.begin(),
...
TYPED_TEST(VectorTest, MoveAssignment) {
  ...
  EXPECT_EQ(std::to_address(this->empty_vec.begin().base()), std::to_address(v2_data));
  ...
  for (unsigned int vec_idx = 0; vec_idx < this->random_bitvecs.size(); ++vec_idx) {
    ...
    EXPECT_EQ(std::to_address(this->empty_vec.begin().base()), std::to_address(bitvec_base));
    EXPECT_TRUE(std::equal(
        this->empty_vec.begin(),
        this->empty_vec.end(),
        boolvec.begin(),
...
TYPED_TEST(VectorTest, EraseAtBegin1) {
  ...
  for (auto _ = 16; _--;) {
    ...
    EXPECT_TRUE(std::equal(
        ...
        boolvec.end(),
        comparator));
    EXPECT_EQ(bitvec.size(), boolvec.size());
    EXPECT_EQ(
